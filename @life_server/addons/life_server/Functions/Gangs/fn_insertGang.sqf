...
if(count _queryResult != 0) exitWith {
	[[1,"Il existe déjà un gang créé avec ce nom, s'il vous plaît choisissez un autre nom."],"life_fnc_broadcast",_ownerID,false] spawn life_fnc_MP;
...
if(count _queryResult != 0) exitWith {
	[[1,"Vous êtes déjà actif dans un gang, s'il vous plait quittez ce gang en premier."],"life_fnc_broadcast",_ownerID,false] spawn life_fnc_MP;
	[["life_action_gangInUse",nil,missionNamespace],"life_fnc_netSetVar",_ownerID,false] spawn life_fnc_MP;
};
