...
if(_veh == "RDS_Lada_Civ_01") exitWith {true}; //ATV's don't need to require a license anymore.
...
switch (life_veh_shop select 0) do
{
	...
	case "ta_avion": {_ret = license_civ_transaltis;};
	...
	case "reb_car": {_ret = license_civ_rebel;};
	case "reb_car_1": {_ret = license_civ_rebel;};
	case "reb_air": {_ret = license_civ_rebel;};
	case "cop_car": {_ret = true;};
    case "cop_car_civ": {_ret = true;};
	case "cop_ship": 
	{
