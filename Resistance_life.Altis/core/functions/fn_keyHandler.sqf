		...
		{
			{
				...
				if(_val > 0 ) then
				{
					if( _str == "Pickaxe" || _str == "pickaxe" || _str == "pioche" ) then
					{
...
hint "Le téléphone de cette personne a été mis par terre.";
...
case 80:
{
	if(_shift) then {_handled = true;};
		if ((_shift) && (vehicle player == player)) then
		{
			cutText [format["Tranquille ma guelle~"], "PLAIN DOWN"];
	 ...
	 case 62:
{
    ...
    diag_log format ["SERVEUR Resistance: %1 utilise ALT+F4 pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]];
    [[1,format["SERVEUR Resistance: %1 utilise ALT+F4 pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
	SOCK_fnc_updateRequest;
    };
};
    ...
    case 211:
{
    ...
    {
	SOCK_fnc_updateRequest;
    diag_log format ["SERVEUR Resistance: %1 utilise CTRL + ALT + DEL pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]];
    [[1,format["SERVEUR Resistance: %1 utilise CTRL + ALT + DEL  pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
	SOCK_fnc_updateRequest;
    };
};
    ...
    case 1:
{
	...
	{
	SOCK_fnc_updateRequest;
    diag_log format ["SERVEUR Resistance: %1 utilise CTRL + ESC pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]];
    [[1,format["SERVEUR Resistance: %1 utilise CTRL + ESC pour se déconnecter (Prenez une capture et signalez le sur le forum)",_player getVariable["realname",name _player]]],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
       SOCK_fnc_updateRequest;
	};
};
};
