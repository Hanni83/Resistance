...
#define iconID 78000
#define scale 0.8
_goggles = ["G_Balaclava_blk","G_Balaclava_lowprofile","G_Balaclava_combat","G_Balaclava_oli","G_Bandanna_aviator","G_Bandanna_tan","G_Bandanna_blk","G_Bandanna_oli","G_Bandanna_shades","Mask_M40","Mask_M40_OD","jakdo_balaclava_BLK","jakdo_balaclavas_oliv","G_jakdo_Band_Us3c1","G_jakdo_Band_winter1","G_Balaclava_AutOliv"];
_uniform = ["U_I_GhillieSuit","U_O_GhillieSuit","U_B_GhillieSuit"];
_headgear = ["H_Shemag_olive","H_Shemag_khk","H_Shemag_olive_hs","H_ShemagOpen_tan","kio_skl_msk_red","DIS_Cobra_Mask","DIS_Camo_Flames_Mask"];
...

{
	...
	if(!(lineIntersects [eyePos player, eyePos _x, player, _x]) && {!isNil {_x getVariable "realname"}}) then {
		...
		if(count _sPos > 1 && {_distance < 15}) then {
			_text = switch (true) do {
				case ((goggles _x) in _goggles): {format["<t color='#000000'>Joueur Masqué</t>"];};
				case ((headgear _x) in _headgear): {format["<t color='#000000'>Joueur Masqué</t>"];};
				case ((uniform _x) in _uniform): {format["<t color='#000000'>Joueur Masqué</t>"];};
				case (_x in (units grpPlayer) && playerSide == civilian): {format["<t color='#00FF00'>%1</t>",(_x getVariable ["realname",name _x])];};
				case (!isNil {(_x getVariable "rank")}): {format["<img image='%1' size='1'></img> %2",switch ((_x getVariable "rank")) do {
					case 2: {"\a3\ui_f\data\gui\cfg\Ranks\corporal_gs.paa"}; 
					case 3: {"\a3\ui_f\data\gui\cfg\Ranks\sergeant_gs.paa"};
